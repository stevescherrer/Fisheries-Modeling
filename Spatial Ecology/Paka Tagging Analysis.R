#### Analysis of spatio-temporal data for P. filamentosus 
...
get_bathymetry = function(region, resolution = 'low'){
  ...
  if(tolower(region) == 'makapuu'){
    ...
    bathymetry = subsetBathy(bathymetry, x = c(-158.49,-157.40), y = c( 21.175, 21.81), locator = FALSE)
...
get_brfa_areas = function(target_depth = -120, target_depth_contour = c(-100, -400), save_plot = TRUE){
  ...
  bathy_50m[is.nan(bathy_50m)] = 100
  
  ## Somewhere for plots to dump out
  if(save_plot){
    png(file.path(results_dir, 'BRFA Habitat Areas.png'), width = 11, height = 8.5, units = "in", res = 600)
    par(mfrow = c(2, 4)) #, bg = '#222222', col.axis = 'grey', col = 'grey', col.lab = 'black', fg = 'grey')
  }
  ...
  plot(brfa, col = c(black, black), land = TRUE, 
       image = TRUE, bpal = list(c(0,max(brfa),green),
                                 c(min(brfa),-400,black, grey),
                                 c(-400,-100, blue),
                                 c(-100, 0, grey, white)),
       shallowest.isobath = 0, 
       deepest.isobath = -4000, 
       step = 100, lty = c(1, 1), 
       drawlabels = TRUE, main = 'BRFA A')
  ...
  lapply(out1, lines, col = "red", lwd = 2, lty = 1)
  ...
  if(min(brfa) >= min(target_depth_contour) & max(brfa) <= max(target_depth_contour)){
    plot(brfa, col = c(black, black), land = TRUE, 
         image = TRUE, bpal = list(c(0,max(brfa),green),
                                  # c(-1000,-400,black, grey),
                                   c(min(brfa),-100, blue),
                                   c(-100, -1, grey, white)),
         shallowest.isobath = 0, 
         deepest.isobath = -4000, 
         step = 100, lty = c(1, 1), 
         drawlabels = TRUE, main = 'BRFA H')
    ### THIS WHOLE BRFA IS FUCKING PROTECTED HABITAT.
    start = as.numeric(c(min(rownames(brfa)), mean(as.numeric(colnames(brfa)))))
    end = as.numeric(c(max(rownames(brfa)), mean(as.numeric(colnames(brfa)))))
  ...
  # plot.bathy(brfa, land = TRUE, image=TRUE,bpal = list(c(0,max(brfa),"#8BBE62"),
  #                                                      c(min(brfa),-400,"black", "grey"),
  #                                                      c(-400,-100, "lightblue"),
  #                                                      c(-100, 0, "grey", "white")),
  #            deepest.isobath = c(-10000), shallowest.isobath = c(-1), step = c(100), drawlabels = TRUE, main = 'BRFA J')
  # # West to East
  # start = as.numeric(c(max(rownames(brfa)), names(brfa[nrow(brfa), ])[which.min(abs(brfa[1, ] - target_depth))]) )# West
  # end = as.numeric(c(min(rownames(brfa)), names(brfa[1, ])[which.min(abs(brfa[nrow(brfa), ] - target_depth))])) # East
  # 
  ...
  ### ALL BRFA DIMENSIONS!!!!
  brfa_dimensions = c(brfa_a_path, brfa_b_path, brfa_d_path, brfa_e_path, brfa_g_path, brfa_h_path, brfa_k_path, brfa_m_path) # brfa_c_path, brfa_l_path, brfa_j_path, brfa_f_path)
...
determine_track_status = function(vue_df, tag_ids = NULL, bathymetry = get_bathymetry('mhi', 'low')){
  # We'll be using the transition matrix for our bathymetry frequently. 
  # Instead of loading it fresh every loop, if we declare it outside of our parallel loop, it only needs to be calculated once
  trans1 = trans.mat(bathymetry, min.depth = -1, max.depth = -10000)
  ## Looping through each individual fish
  if(is.null(tag_ids)){
    tag_ids = unique(vue_df$tag_id)
  }
  
  mortality_status = foreach(i = 1:length(tag_ids), .combine = rbind) %dopar% {
    
    indv_data = vue_df[vue_df$tag_id == tag_ids[i], ]
    
    rationalle = c()
    
    if(dim(indv_data)[1] == 0){
      rationalle = c(rationalle, 'No Tag Detections')
      status = 'Not Detected'
    } else {
      ...
      if(any(lc.dist(trans1, locations, res = "dist") > 2.2)){
        ...
        rationalle = paste(rationalle, 'Observed Movement More Than 2km')
          ...
          if (!is.na(absolute_range)){ # If yes...
            ...
            status = 'Dead'
            rationalle = paste(rationalle, 'Tag detected at 4 or more stations in first 14 days', sep = ',')
            ### If it's not, we don't know what's happening.
          } else {
            status = 'Unknown'
            rationalle = paste(rationalle, 'Tag detected at < 4 stations in first 14 days', sep = ',')
          }
        }
    # If the tag wasn't detected 14 days after tagging, we ommit it from our analysis'    
      } else {
        status = 'Excluded From Analysis'
        rationalle = paste(rationalle, 'Tag Detected < 14 days', sep = ',')
        
        # We're still interested in knowing how many of these tags moved a lot vs. a little, so we'll further classify here.
        ...
        # Was the fish detected inside or outside the BRFA?
    ...
    # Return each fish's individual results in a data frame.
    write_df_line = data.frame('tag_id' = tag_ids[i], 'status' = status, 'rationalle' = rationalle )
    return(write_df_line)
  }
  
  ## Order the dataframe containing our loop results by tag ID and write it out to our results directory
  ...
  ## Create a list for returning tag IDs by status
  track_status = list()
...
plot_detection_records = function(vue_df, receiver_df = receiver_df, start_date = as.POSIXct("2017-06-26 HST"), end_date = NULL, color_palette = NULL, bathymetry = NULL){
...
six_pannel_plot = function(analysis_summary, vue_df = NULL, start_date = NULL, end_date = NULL, color_palette = NULL){
  ...
  bathy = makapuu_bathy
  ### Plotting empty map corosponding to station colors
  png(file.path(results_dir, 'Station Plot - Colored.png'), width = 318*2, height = 279*2, units = 'px')
  ...
  receivers_to_plot = receiver_df[receiver_df$recovery_date >= min(analysis_summary$data$datetime) & receiver_df$deployment_date <= max(analysis_summary$data$datetime) & !is.na(receiver_df$station_name), ]
  ...
  points(lat~lon, data = receivers_to_plot[receivers_to_plot$recovered == "" & receivers_to_plot$station_name %in% analysis_summary$data$station , ], col = receivers_to_plot$plot_color[receivers_to_plot$recovered == "" & receivers_to_plot$station_name %in% analysis_summary$data$station], pch = 19, cex = 1.5)
  ## Hollow points for stations without detections
  points(lat~lon, data = receivers_to_plot[receivers_to_plot$recovered == "" & !receivers_to_plot$station_name %in% analysis_summary$data$station , ], col = receivers_to_plot$plot_color[receivers_to_plot$recovered == "" & !receivers_to_plot$station_name %in% analysis_summary$data$station], pch = 1, cex = 1.5)
  ...
  for(i in 1:length(unique(analysis_summary$tag_ids))){
    print('')
    print(paste('Plot', i, 'of', length(unique(analysis_summary$tag_ids))))
    print(unique(analysis_summary$tag_ids)[i])
    
    png(paste(unique(analysis_summary$tag_ids)[i], '6 pannel plot.png'), height = 11, width = 8.5, units = 'in', res = 600)
    ...
    indv_data = vue_df[vue_df$tag_id == analysis_summary$tag_ids[i], ]
    
    # if(analysis_summary$summary_df$tagging_date[analysis_summary$summary_df$tag_id == unique(analysis_summary$tag_ids)[i]] <= as.POSIXct('2015-03-17 0:00:00 HST')){
    #   trans1 = trans_pb
   #  bathy = pb_bathy
    # } else if (analysis_summary$summary_df$tagging_date[analysis_summary$summary_df$tag_id == unique(analysis_summary$tag_ids)[i]] < ('2015-05-30')) {
    #   trans1 = trans_oahu
   # bathy = oahu_bathy
    # } else {
    ...
    plot.new()
    # Tag ID
    text(x = 0, y = .95, paste('Tag ID:', analysis_summary$tag_ids[i]), pos = 4, cex = 1.5)
    
    # text(x = 0.5, y = .95, paste('Species:', tagging_data$species[tagging_data$vem_tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    # Track Status
    text(x = 0, y = .85, paste('Track Status:', analysis_summary$track_status$status_df$status[analysis_summary$track_status$status_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    # text(x = 0, y = .65, paste('Track Status:', analysis_summary$track_status$status_df$rationalle[analysis_summary$track_status$status_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = .25)
    
    # Tagging Date
    text(x = 0, y = .75, paste('Tagging Date:', analysis_summary$summary_df$tagging_date[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    
    # Data Density
    text(x = 0, y = .65, paste('Data Density:', round(as.numeric(analysis_summary$summary_df$data_density[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]]), digits = 2)), pos = 4, cex = 1.5)
    
    # Total detections
    text(x = 0, y = 0.55, paste('Tag Detections:', analysis_summary$summary_df$n_detections[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    
    # Track length
    text(x = 0, y = 0.45, paste('Track Length:', analysis_summary$summary_df$days_at_liberty[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]], 'Days'), pos = 4, cex = 1.5)
    
    # Unique Stations Detected
    text(x = 0, y = 0.35, paste('Unique Locations Detected:', analysis_summary$summary_df$n_stations[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    
    # Movements Detected
    text(x = 0, y = 0.25, paste('Movements Observed:', analysis_summary$summary_df$movements_detected[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    
    # BRFA Crossings
    text(x = 0, y = 0.15, paste('BRFA Crossings Observed:', analysis_summary$summary_df$total_brfa_crossings[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]]), pos = 4, cex = 1.5)
    
    # maximum observed movement
    text(x = 0, y = 0.05, paste('Maximum Linear Movement:', round(as.numeric(analysis_summary$summary_df$z_constrained_path_distance[analysis_summary$summary_df$tag_id == analysis_summary$tag_ids[i]], 'km'), digits = 2)), pos = 4, cex = 1.5)
    ...
    ## Hollow points for stations without detections
    points(lat~lon, data = receivers_to_plot[receivers_to_plot$recovered == "" & !receivers_to_plot$station_name %in% indv_data$station , ], col = receivers_to_plot$plot_color[receivers_to_plot$recovered == "" & !receivers_to_plot$station_name %in% indv_data$station], pch = 1, cex = 1.5)
    
    ### Adding movement paths to map
    points(lat ~ lon, data = analysis_summary$tagging_df[analysis_summary$tagging_df$vem_tag_id == analysis_summary$tag_ids[i], ], col = 'yellow', pch = 17, cex = 1.5)
    indv_locations = as.data.frame(rbind(cbind(analysis_summary$tagging_df$lon[analysis_summary$tagging_df$vem_tag_id %in% analysis_summary$tag_ids[i]], analysis_summary$tagging_df$lat[analysis_summary$tagging_df$vem_tag_id %in% analysis_summary$tag_ids[i]]), cbind(indv_data$lon, indv_data$lat)))
    ...
    points(lat ~ lon, data = analysis_summary$tagging_df[analysis_summary$tagging_df$vem_tag_id == analysis_summary$tag_ids[i], ], col = 'yellow', pch = 17, cex = 1.5)
    lines(lat ~ lon, data = indv_data, col = 'black', lwd = 3)
...
dissertation_H1 = function(analysis_summary, save_plot = TRUE){
  ...
  if(length(analysis_summary$homerange$max_polygon_area[!is.na(analysis_summary$homerange$max_polygon_area)]) > 0){
    H1_areas_sqr = as.data.frame(rbind(cbind(analysis_summary$homerange$max_polygon_area[!is.na(analysis_summary$homerange$max_polygon_area)], "P. filamentosus"), cbind(brfa_areas_km2, "BRFA")), stringsAsFactors = FALSE)
    colnames(H1_areas_sqr) = c("area", "source")
    rownames(H1_areas_sqr) = NULL
    H1_areas_sqr$area = as.numeric(H1_areas_sqr$area)
    
    H1_area_plot = ggplot(data = H1_areas_sqr, aes(y = area, x= source)) + 
      geom_boxplot() +
      labs(y = 'km^2', x = 'Source')
    
    if(save_plot){
      pdf(file = "H1 - Boxplot - Homerange vs BRFA Area KM2.pdf")
    }
    print(H1_area_plot)
    if(save_plot){
      dev.off()
  ...
  H1_areas_linear = as.data.frame(rbind(cbind(sqrt(analysis_summary$homerange$max_linear_distance), "P. filamentosus"), cbind(sqrt(brfa_areas_km2), "BRFA")), stringsAsFactors = FALSE)
  colnames(H1_areas_linear) = c("area", "source")
  rownames(H1_areas_linear) = NULL
  H1_areas_linear$area = as.numeric(H1_areas_linear$area)
  
  H1_linear_plot = ggplot(data = H1_areas_linear, aes(y = area, x= source)) + 
    geom_boxplot() +
    labs(y = 'km', x = 'Source')
  
  if(save_plot){
    pdf(file = "H1 - Max Linear Homerange vs. sqrt BRFA Area.pdf")
  }
  print(H1_linear_plot)
  if(save_plot){
    dev.off()
  }
  ...
  H1_areas_constrained = as.data.frame(rbind(cbind(analysis_summary$homerange$z_constrained_path_distance, "P. filamentosus"), cbind(brfa_areas_km2, "BRFA")), stringsAsFactors = FALSE)
  colnames(H1_areas_constrained) = c("area", "source")
  rownames(H1_areas_constrained) = NULL
  H1_areas_constrained$area = as.numeric(H1_areas_constrained$area)
  
  H1_areas_linear$area = as.numeric(H1_areas_linear$area)
  
  H1_constrained_plot = ggplot(data = H1_areas_constrained, aes(y = area, x= source)) + 
    geom_boxplot() +
    labs(y = 'km', x = 'Source')
  
  if(save_plot){
    pdf(file = "H1 - Z-Constrained Homerange vs. Z-Constrained BRFA Area.pdf")
  }
  print(H1_constrained_plot)
  if(save_plot){
    dev.off()
  }
...
dissertation_H2 = function(analysis_summary, save_plot = TRUE){
  ...
  if(save_plot){
    ...
    # Save plot
    pdf(file = 'H2 - Valid Mean BRFA Crossings per Day Hist.pdf')
    print(brfa_crossing_hist)
    dev.off()
    ...
    # Save plot
    pdf(file = 'H2 - Valid Mean Days Between Crossings Hist.pdf')
...
run = function(vue_df, tagging_df, receiver_df, start_date = NULL, end_date = NULL, tag_ids = NULL,  run_description = FALSE, region = "Makapuu", valid_tracks = NULL, questionable_tracks = NULL, expired_tracks = NULL, plot = TRUE, report = TRUE){
  ...
  if(dim(vue_df)[1] > 0){
    ...
    #### Writing out run description
    analysis_summary$run_description = run_description
    ...
    analysis_summary$summary_df$brfa_crossings_to_movements = analysis_summary$summary_df$total_brfa_crossings / analysis_summary$summary_df$movements_detected
    analysis_summary$summary_df$percent_days_out = round(analysis_summary$summary_df$days_tracked_out / (analysis_summary$summary_df$days_tracked_out + analysis_summary$summary_df$days_tracked_in), digits = 2)
    analysis_summary$summary_df$percent_days_in = round(analysis_summary$summary_df$days_tracked_in / (analysis_summary$summary_df$days_tracked_out + analysis_summary$summary_df$days_tracked_in), digits = 2)
    analysis_summary$summary_df$spatial_evenness = round(calculate_spatial_evenness(vue_df = analysis_summary$data, receiver_df = analysis_summary$receiver_df), digits = 2)
    
    ################## Answering dissertation Hypotheses #################
    print('UGH... DISSERTATION QUESTIONS?!')
    #analysis_summary$h1_results = dissertation_H1(analysis_summary, save_plot = plot)
